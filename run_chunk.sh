...
OUTPUT_BASE="k${k}_m${m}_b${b}_s${s}_xy${xy}_chunk${chunk_size}"
...
for x in "${x_values[@]}"; do
    echo -e "\n[$(date '+%H:%M:%S')] Starting execution: $EXECUTABLE -v -f $f -k $k -b $b -s $s -o "${RESULTS_DIR}/${OUTPUT_BASE}_x$x.csv" -x $x -a ${xy} -c -z ${chunk_size}"
    ...
    $EXECUTABLE -v -f $f -k $k -b $b -s $s -o "${RESULTS_DIR}/${OUTPUT_BASE}_x$x.csv" -x $x -a ${xy} -c -z ${chunk_size} > "$LOG_FILE" 2>&1
